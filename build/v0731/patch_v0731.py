...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise RuntimeError(f"missing v0.7.3.1 patch anchor: {label}")
...
def patch(source_root: Path, repo_root: Path):
    ...
    if not main_path.exists() or not brain_path.exists() or not voice_src.exists():
        raise FileNotFoundError("v0.7.3.1 source inputs missing")
    ...
    s, n = re.subn(
        r'APP_NAME\s*=\s*"[^"]*"\s*\nAPP_VERSION\s*=\s*"0\.7\.2"',
        'APP_NAME = "小美丽 V0.7.3.1｜Result Screen 2026 + Clean Chat + Girl Voices"\nAPP_VERSION = "0.7.3.1"',
        s,
        count=1,
    )
    if n != 1:
    ...
    s = replace_once(s, '"config_version": 20,', '"config_version": 21,', "config version")

    s = replace_once(
        s,
        'ROI_REPORT_CONTINUE = (0.410, 0.840, 0.590, 0.940)',
        'ROI_REPORT_CONTINUE = (0.405, 0.900, 0.595, 0.990)',
        "2026 result screen button ROI",
    )
    s = replace_once(
        s,
        'return (red_frac >= 0.16 and gray_votes >= 4), float(score)',
        'return (red_frac >= 0.10 and gray_votes >= 4), float(score)',
        "2026 result screen red threshold",
    )

    s, n = re.subn(
        r'\n\s*self\.brain_chat\.addItem\(f"〔白板草稿｜\{emotion\}〕\{board\.replace\(chr\(10\),\' / \'\)\}"\)',
        '',
        s,
        count=1,
    )
    if n != 1:
        raise RuntimeError("clean chat display replacement failed")

    anchor = '''        self.voice_prepare_btn.clicked.connect(self._prepare_voice_components)
        vf2.addRow(self.voice_prepare_btn)

        self.voice_combo'''
    replacement = '''        self.voice_prepare_btn.clicked.connect(self._prepare_voice_components)
        vf2.addRow(self.voice_prepare_btn)
        ...
        self.voice_combo'''
    s = replace_once(s, anchor, replacement, "VoiceDesign button")

    handler_anchor = '    def _prepare_voice_components(self):\n'
    handler = '''    def _prepare_voice_design(self):
...
'''
    new_finish = '''        self.voice_status.setText(str(message))
        ...
        if ok:
            ...
            self.voice_service.load_voices_async()
'''
    s = replace_once(s, old_finish, new_finish, "voice setup finished")

    startup_old = 'self.hotkeys.register(cfg); self.pet.show(); self.vision.start(); QTimer.singleShot(5000, cleanup_obsolete_storage)'
    startup_new = (
        'self.hotkeys.register(cfg); self.pet.show(); self.vision.start(); '
        'QTimer.singleShot(5000, cleanup_obsolete_storage); '
        'QTimer.singleShot(9000, lambda: self.brain_service.cleanup_ndm_leftovers('
        'str(self.cfg.get("brain", {}).get("ndm_download_dir", "") or "")))'
    )
    s = replace_once(s, startup_old, startup_new, "NDM leftover cleanup schedule")

    migration_anchor = '    cfg["brain"].update({k: v for k, v in old_brain.items() if k in cfg["brain"]})\n'
    migration = '''    cfg["brain"].update({k: v for k, v in old_brain.items() if k in cfg["brain"]})
    if "spoken_text" in str(cfg["brain"].get("persona", "")) and "board_text" in str(cfg["brain"].get("persona", "")):
        cfg["brain"]["persona"] = DEFAULT_PERSONA
'''
    s = replace_once(s, migration_anchor, migration, "persona migration")

    s = s.replace('QGroupBox("小美丽人格卡")', 'QGroupBox("小美丽性格卡（只影响她怎么说话）")', 1)

    main_path.write_text(s, encoding="utf-8")

    b = brain_path.read_text(encoding="utf-8")

    persona_start = b.index('DEFAULT_PERSONA = """')
    persona_end = b.index('"""', persona_start + len('DEFAULT_PERSONA = """')) + 3
...
说中文时像中国小女孩的自然口语，不要客服腔，不要写作文，不要自称AI。
...
JSON只允许三个字段：
spoken_text：真正说出口的中文回答；
board_text：最多两行的中文短句，供以后白板使用；
emotion：只能是 neutral、happy、teasing、proud、annoyed、concerned 之一。
spoken_text 绝不能包含 spoken_text、board_text、emotion 这些字段名。
普通闲聊的 spoken_text 默认一句话，短、自然、中文优先。
"""
'''
    b = b[:persona_start] + new_persona + b[persona_end:]

    norm_start = b.index('def _normalize_answer(raw: str):')
    norm_end = b.index('\n\n\nclass BrainService', norm_start)
    new_norm = r'''def _clean_field_value(value: str):
    value = str(value or "").strip().strip(chr(96)).strip().strip('"').strip("'")
    for marker in ("board_text", "emotion", "spoken_text"):
        m = re.search(rf"(?im)^\s*{re.escape(marker)}\s*[:：=]", value)
        if m and m.start() > 0:
            value = value[:m.start()].strip()
    value = re.sub(r"(?i)^\s*(spoken_text|spoken text)\s*[:：=]\s*", "", value).strip()
    return value


def _loose_field(text: str, key: str):
    pattern = rf"(?ims)(?:^|[\n{{,])\s*[\"']?{re.escape(key)}[\"']?\s*[:：=]\s*[\"']?(.*?)(?=\n\s*[\"']?(?:spoken_text|board_text|emotion)[\"']?\s*[:：=]|\Z)"
...
'''
    b = b[:norm_start] + new_norm + b[norm_end:]

    b = replace_once(
        b,
        '                system = str(persona or DEFAULT_PERSONA).strip()\n',
        '                system = str(persona or DEFAULT_PERSONA).strip() + "\\n\\n" + OUTPUT_CONTRACT.strip()\n',
        "internal JSON output contract",
    )
    ...
    cleanup_method = '''    def cleanup_ndm_leftovers(self, download_root):
        ...
        for p in list(root.rglob("*")):
            ...
            try:
                ...
                removed += 1
                reclaimed += size
                LOGGER.info("Removed XiaoMeili NDM leftover: %s (%.1f MB)", p, size / 1024 / 1024)
            except Exception:
                LOGGER.warning("Failed to remove NDM leftover: %s", p, exc_info=True)
...
'''
    ...
    print("Patched XiaoMeili source to V0.7.3.1")
